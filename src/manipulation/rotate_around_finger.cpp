...
BT::NodeStatus MoveEEF::tick()
{
    ...
    else if (controller_name.find("left") != std::string::npos)
    {
        prefix = "left";
    }
    else
    {
        throw BT::RuntimeError(
            ...
            "' does not specify a valid arm side (expected to contain 'right' or 'left').");
    }

    geometry_msgs::msg::PoseStamped pose_msg;
    try {
        pose_msg = tf_wrapper_->lookupTransform(
            "map", prefix + "_arm_wrist_3_link", rclcpp::Duration::from_seconds(2.0));
    ...
    request->name_target = { 
        prefix + "_arm_shoulder_pan_joint", 
        prefix + "_arm_shoulder_lift_joint", 
        prefix + "_arm_elbow_joint", 
        prefix + "_arm_wrist_1_joint", 
        prefix + "_arm_wrist_2_joint", 
        prefix + "_arm_wrist_3_joint" 
    };
    request->ik_link = prefix + "_arm_wrist_3_link";
